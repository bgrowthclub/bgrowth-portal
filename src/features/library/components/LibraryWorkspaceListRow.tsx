...
interface LibraryWorkspaceListRowProps {
  ...
  /** This member's review for this Workspace, batched by the caller (see MyLibraryPage/reviewService.getForUserBatch) — undefined while that batch fetch is still loading. Same data, same eligibility rule as LibraryWorkspaceCard's Grid View. */
 ...
 * a dense list rather than a full card per row. Drops what the card shows
 * beyond the essentials (trial-expiry copy, the SavedChecklistsPanel) in
 * exchange for density, but the Review/Product Evaluation prompt is still
 * available here too — same ReviewPromptCard, same eligibility rule as Grid
 * View, just rendered as its own row beneath the compact one instead of
 * inline in it.
 */
export function LibraryWorkspaceListRow({
  ...
  return (
    <div className="card flex flex-col gap-3 p-4">
      ...
      {/* Same eligibility rule as LibraryWorkspaceCard's Grid View: any
          access state the member has (or had) except "locked" (never
          activated/bought — this row never renders for that state, see
          MyLibraryPage). Its own row beneath the compact one above, rather
          than squeezed into that single-line layout. */}
      {(canOpen || isExpired) && (
        <div className="border-t border-navy-100/60 pt-3 dark:border-white/10">
          <ReviewPromptCard
            userId={userId}
