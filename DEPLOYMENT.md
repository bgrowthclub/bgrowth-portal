...
3. Before the first deploy, add these **Environment Variables** (Project
   ...
   | `EMAIL_FROM_ADDRESS` | Optional — defaults to `info@bgrowth.app`, BGrowth's production sender. **Must** be on a domain verified in Resend (Domains tab), or every send is rejected. Resend's own sandbox address only delivers to the account owner, never real members. Change this (and redeploy) to switch senders later — with no template or code changes needed |
   ...
   | `SUPPORT_EMAIL` | the real, monitored BGrowth support inbox (`info@bgrowth.app`) — powers only the "Support" footer link every notification email renders (`api/_lib/email/layout.ts`). No longer affects reply-to — see `EMAIL_REPLY_TO` above. Without it, the footer link is simply omitted |
